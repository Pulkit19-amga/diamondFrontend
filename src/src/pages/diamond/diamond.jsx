...
export default function Diamond() {
  ...
  const fetchDiamonds = async (pageNumber) => {
    try {
      setLoading(true);
      setError("");
      // const response = await axiosClient.get("/api/get-all-diamonds");
      const response = await axiosClient.get(
        `/api/get-all-diamonds?page=${pageNumber}&per_page=50`
      );
      ...
      setDiamonds((prev) => [...prev, ...responseData.data]);
    ...
    <>
      ...
      {hasMore && !loading && (
        <div style={{ textAlign: "center", margin: "1rem" }}>
          <button onClick={() => fetchDiamonds(page)}>Load More</button>
        </div>
      )}
    </>
  );
